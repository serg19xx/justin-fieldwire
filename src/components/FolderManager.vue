<template>
      ...
      <div class="flex items-center space-x-2">
        ...
        <div class="flex items-center space-x-1">
          <button
            ...
            class="px-3 py-1 text-xs rounded-l hover:bg-blue-600 transition-colors"
            title="Icons View"
          >
            🗂️
          </button>
          <button
            ...
            class="px-3 py-1 text-xs rounded-r hover:bg-blue-600 transition-colors"
            title="Details View"
          >
            📋
          </button>
    ...
    <div class="flex-1 flex min-h-0">
      ...
      <div class="flex-1 flex flex-col min-h-0" style="width: calc(100% - 256px);">
        ...
        <div class="flex-1 overflow-y-auto overflow-x-auto p-3">
          ...
          >
            ...
            >
              <div class="flex flex-col items-center text-center h-full">
                ...
                <h3 class="text-xs font-medium text-gray-900 truncate w-full flex-shrink-0" :title="folder.name">
                  {{ folder.name }}
                </h3>
              </div>
            </div>
          ...
          <!-- Details View -->
          <div v-else-if="viewMode === 'details'" class="flex flex-col h-full overflow-hidden">
            <!-- Content with Scroll -->
            <div v-if="currentItems.length > 0" class="flex-1 overflow-auto border border-gray-300" style="width: 100%;">
              <div style="width: 1200px; min-width: 1200px;">
                <!-- Header -->
                <div style="display: flex; background-color: #f5f5f5; border-bottom: 1px solid #ddd; width: 1200px;">
                  <div style="width: 400px; padding: 8px; font-weight: bold; color: #374151;">Name</div>
                  <div style="width: 100px; padding: 8px; text-align: right; font-weight: bold; color: #374151;">Size</div>
                  <div style="width: 120px; padding: 8px; text-align: right; font-weight: bold; color: #374151;">Type</div>
                  <div style="width: 180px; padding: 8px; text-align: right; font-weight: bold; color: #374151;">Modified</div>
                  <div style="width: 300px; padding: 8px; font-weight: bold; color: #374151;">Description</div>
                  <div style="width: 100px; padding: 8px; text-align: right; font-weight: bold; color: #374151;">Actions</div>
                </div>
                ...
                <div
                  ...
                  :class="
                    selectedItems.some((item) => item.id === folder.id && item.type === 'folder')
                      ...
                      : 'hover:bg-gray-50'
                  "
                  style="display: flex; cursor: pointer; border-bottom: 1px solid #eee; width: 1200px;"
                >
                  <div style="width: 400px; padding: 8px; display: flex; align-items: center;">
                    <span style="margin-right: 12px; font-size: 18px;">{{ getFolderIcon(folder.name) }}</span>
                    <span style="font-weight: 500; color: #111827; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; flex: 1;" :title="folder.name">{{ folder.name }}</span>
                  </div>
                  <div style="width: 100px; text-align: right; padding: 8px; color: #6b7280;"></div>
                  <div style="width: 120px; text-align: right; padding: 8px;">
                    <span style="background: #f3f4f6; color: #374151; padding: 2px 8px; border-radius: 12px; font-size: 12px; font-weight: 500;">Folder</span>
                  </div>
                  <div style="width: 180px; text-align: right; padding: 8px; color: #6b7280;">{{ formatDate(folder.created_at) }}</div>
                  <div style="width: 300px; padding: 8px; color: #6b7280;"></div>
                  <div style="width: 100px; text-align: right; padding: 8px;">
                    <button
                      @click.stop="startRename({ id: folder.id, type: 'folder', name: folder.name })"
                      style="color: #2563eb; padding: 4px; border-radius: 4px; background: none; border: none; cursor: pointer;"
                      title="Rename folder"
                    >
                      ✏️
                    </button>
                ...
                >
                  <div style="width: 400px; padding: 8px; display: flex; align-items: center;">
                    <span style="margin-right: 12px; font-size: 18px;">{{ getFileIcon(file.mime_type, file.file_name).icon }}</span>
                    <span style="font-weight: 500; color: #111827; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; flex: 1;" :title="file.file_name">{{ truncateFileName(file.file_name, 35) }}</span>
                  </div>
                  <div style="width: 100px; text-align: right; padding: 8px; color: #111827;">{{ formatFileSize(file.file_size) }}</div>
                  <div style="width: 120px; text-align: right; padding: 8px;">
                    <span
                  ...
                  <div style="width: 180px; text-align: right; padding: 8px; color: #6b7280;">{{ formatDate(file.updated_at) }}</div>
                  <div style="width: 300px; padding: 8px; color: #6b7280;">
                    <span v-if="file.description" style="color: #6b7280;">{{ file.description.length <= 30 ? file.description : file.description.substring(0, 30) + '...' }}</span>
                    <span v-else style="color: #9ca3af; cursor: pointer;" @click.stop="startDescriptionEdit({ id: file.id, type: 'file', name: file.file_name, description: file.description })">Click to add</span>
                  </div>
                  <div style="width: 100px; text-align: right; padding: 8px;">
                    <button
                      @click.stop="startDescriptionEdit({ id: file.id, type: 'file', name: file.file_name, description: file.description })"
                      style="color: #16a34a; padding: 4px; border-radius: 4px; background: none; border: none; cursor: pointer; margin-right: 4px;"
      ...
      <div class="px-6 py-4">
        ...
        <div class="text-sm text-gray-600">
          ...
          <ul class="mt-2 space-y-1">
            <li v-for="item in moveItems" :key="`${item.type}-${item.id}`" class="flex items-center">
  ...
  >
    <div class="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
      <div class="px-6 py-4 border-b border-gray-200">
        <h3 class="text-lg font-semibold text-gray-900">Rename {{ editItem?.type === 'file' ? 'File' : 'Folder' }}</h3>
        <p class="text-sm text-gray-600 mt-1">
          Enter new name for "{{ editItem?.currentName }}"
        </p>
      </div>
      <div class="px-6 py-4">
        <div class="mb-4">
...
// View mode: 'icons' | 'details'
const viewMode = ref<'icons' | 'details'>('icons')

// Edit dialogs state
const showRenameDialog = ref(false)
const showDescriptionDialog = ref(false)
const editItem = ref<{ id: number; type: 'file' | 'folder'; currentName: string; currentDescription?: string } | null>(null)
const newName = ref('')
...
async function downloadFile(file: FileUpload) {
  try {
    ...
    if (error && typeof error === 'object' && 'response' in error) {
      ...
      if (axiosError.response?.status === 404) {
        alert(`File not found on server. File ID: ${file.id}. The file may have been deleted or moved.`)
      } else {
...
function startRename(item: { id: number; type: 'file' | 'folder'; name: string }) {
  editItem.value = {
    id: item.id,
    type: item.type,
...
function startDescriptionEdit(item: { id: number; type: 'file' | 'folder'; name: string; description?: string }) {
  editItem.value = {
    id: item.id,
    type: item.type,
    currentName: item.name,
...
async function executeRename() {
  if (!editItem.value || !newName.value.trim()) return

  try {
    if (editItem.value.type === 'file') {
      console.log('🔄 Renaming file:', editItem.value.id, 'to:', newName.value.trim())
      const updatedFile = await filesApi.renameFile(editItem.value.id, newName.value.trim())
      console.log('📄 API response:', updatedFile)
      // Update local file data
      const fileIndex = files.value.findIndex(f => f.id === editItem.value!.id)
      if (fileIndex !== -1) {
        files.value[fileIndex] = updatedFile
        console.log('✅ File updated in local array')
      } else {
        console.log('⚠️ File not found in local array, refreshing...')
      }
      console.log('✅ File renamed:', updatedFile.file_name)
    } else {
      const updatedFolder = await filesApi.renameFolder(editItem.value.id, newName.value.trim())
      // Update local folder data - need to find in the hierarchical structure
      const updateFolderInTree = (folderList: Folder[], folderId: number, updatedFolder: Folder): boolean => {
        for (let i = 0; i < folderList.length; i++) {
          if (folderList[i].id === folderId) {
            folderList[i] = updatedFolder
            return true
          }
          if (folderList[i].children && updateFolderInTree(folderList[i].children!, folderId, updatedFolder)) {
            return true
          }
        }
        return false
      ...
      updateFolderInTree(folders.value, editItem.value.id, updatedFolder)
    ...
    alert(`❌ Error during rename: ${errorMessage}`)
  }
}
...
async function executeDescriptionUpdate() {
  ...
  try {
    const updatedFile = await filesApi.updateFileDescription(editItem.value.id, newDescription.value.trim())
    // Update local file data
    const fileIndex = files.value.findIndex(f => f.id === editItem.value!.id)
    if (fileIndex !== -1) {
      files.value[fileIndex] = updatedFile
...
function moveSelected() {
  ...
  const allFolders = getAllFoldersFlat()
  console.log('📁 All available folders:', allFolders.map(f => ({ id: f.id, name: f.name, parent_id: f.parent_id })))
...
async function executeMove() {
  ...
  try {
    console.log('📁 Executing move operation:', moveItems.value, 'to folder:', moveDestinationFolderId.value)
    ...
    for (const item of moveItems.value) {
      if (item.type === 'file') {
        ...
        // Move folder via API
        console.log('📁 Moving folder via API:', item.id, 'to parent:', moveDestinationFolderId.value)
    ...
    if (error && typeof error === 'object' && 'response' in error) {
      ...
      if (axiosError.response?.status === 500) {
        alert(`❌ Move operations are not yet implemented on the backend.\n\nPlease wait for backend implementation.\n\nError: ${axiosError.message || 'Unknown error'}`)
      } else {
...
function getFolderPath(folder: Folder): string {
  ...
  console.log('🔍 Building path for folder:', folder.name, 'ID:', folder.id, 'Parent ID:', folder.parent_id)
