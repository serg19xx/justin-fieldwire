...
const filteredWorkers = computed(() => {
  ...
  // Фильтр по навыкам (если есть поле skills)
  if (skillFilter.value) {
    filtered = filtered.filter((worker) =>
      worker.job_title?.toLowerCase().includes(skillFilter.value.toLowerCase()),
    )
  }
  ...
  if (statusFilter.value) {
    if (statusFilter.value === 'available') {
      filtered = filtered.filter((worker) => worker.status === 1)
    } else if (statusFilter.value === 'busy') {
      filtered = filtered.filter((worker) => worker.status === 0)
    }
  }
...
async function sendInvitations() {
  ...
  try {
    console.log('📧 Sending invitations to:', selectedWorkers.value.length, 'workers')

    // Здесь будет логика отправки приглашений
    // Пока что просто эмитим событие
...
// Watch для перезагрузки при изменении фильтров
watch([searchQuery, skillFilter, statusFilter], () => {
  // Фильтрация происходит в computed, перезагрузка не нужна
})
</script>

<template>
  <div class="pm-resource-manager">
    <!-- Header -->
    <div class="bg-white shadow rounded-lg p-4 mb-4">
      <div class="flex items-center justify-between mb-4">
        <h2 class="text-lg font-semibold text-gray-900">
          {{ mode === 'project' ? 'Project Team Resources' : 'Task Team Resources' }}
        </h2>
      ...
      <div v-if="isFiltersOpen" class="mt-4 pt-4 border-t border-gray-200">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Skill/Job Title</label>
            <input
              v-model="skillFilter"
              type="text"
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr
              ...
              :class="{ 'bg-blue-50': selectedWorkers.some((w) => w.id === worker.id) }"
            >
