  ...
  return builders.value
    ...
    .filter(builder => {
      const matchesSearch = searchQuery.value === '' ||
        ...
        (builder.jobTitle && builder.jobTitle.toLowerCase().includes(searchQuery.value.toLowerCase()))
...
const canViewWorkers = computed(() => {
  const userType = authStore.currentUser?.user_type
  return ['Project Manager', 'System Administrator', 'General Contractor', 'Trade Contractor', 'Architect', 'Client'].includes(userType || '')
})
...
function addWorkerToProject(worker: User) {
  ...
  alert(`Functionality to add ${worker.firstName} ${worker.lastName} to project will be implemented`)
}
        ...
        <div class="flex items-center justify-between space-x-2 sm:space-x-4">
          ...
          <div class="flex items-center space-x-2 sm:space-x-3">
            ...
            <div class="w-48 sm:w-56 md:w-64 relative">
              ...
              />
              <svg class="absolute left-2 sm:left-2.5 top-1.5 sm:top-2 h-3 w-3 sm:h-3.5 sm:w-3.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div class="flex items-start space-x-3">
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center justify-between mb-2">
                    ...
                    >
                      {{ builder.status === 'active' ? 'Active' : builder.status === 'inactive' ? 'Inactive' : 'Pending' }}
                    </span>
                  </div>
