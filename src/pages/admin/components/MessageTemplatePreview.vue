      ...
      >
        ...
        <div class="p-6">
          ...
          <div class="space-y-4">
            ...
            <!-- Variables Used -->
            <div v-if="variableKeys.length > 0" class="border border-gray-200 rounded-lg p-4">
              <h3 class="text-sm font-medium text-gray-700 mb-2">Variables Used</h3>
              <div class="flex flex-wrap gap-2">
                <span
                  v-for="variable in variableKeys"
                  :key="variable"
            ...
            <div v-if="variableKeys.length > 0" class="border border-gray-200 rounded-lg p-4">
              ...
              <div class="bg-gray-50 rounded p-3 text-xs">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  <div v-for="variable in variableKeys" :key="variable">
                    <span class="font-medium">{{ variable }}:</span>
                    <span class="ml-1">{{ sampleData[variable] }}</span>
                  </div>
                </div>
              </div>
            </div>
...
const variableKeys = computed(() =>
  extractTemplateVariableKeys(props.template?.subject, props.template?.body),
)

const sampleData = computed(() =>
  buildTemplateSampleData(variableKeys.value, props.template?.variables || null),
)

const previewSubject = computed(() =>
  renderTemplateString(props.template?.subject || '', sampleData.value),
)

const previewContent = computed(() =>
  renderTemplateString(props.template?.body || '', sampleData.value).replace(/\n/g, '<br>'),
)
